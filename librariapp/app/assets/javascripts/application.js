...
$('#client_rfc').on("keypress",function(event) {
    //buscar el libro por codigo y agregarlo
    if ( event.which == 13)
    {
      agregaCliente($('#client_rfc').val());
    }
    else{
...
function agregaCliente(id){
  ...
  $.ajax({
    dataType: "json",
    url: "http://localhost:3000/clients/find.json?rfc=" + id})
  .done(function(data) {
      if(data.id == null){
     ...
     //  alert("Datos del libro: " + data.id + " codigo: " + data.email);
       // $('#nombre').click(function ()
          ...
          $('#sale_client_id').val(data.id);
          $('#client_nombreCliente').val(data.nombreCliente);
          $('#client_direccion').val(data.direccion);
          $('#client_correo').val(data.correo);
